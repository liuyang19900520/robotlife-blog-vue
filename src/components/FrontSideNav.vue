      ...
      <el-col :span="24" style="width:100%">
        <div class="item-template">
          <el-menu class="el-menu-vertical-demo" @open="handleOpen" @close="handleClose" unique-opened>
            ...
            <el-submenu v-for="(item, index) in categories" :key="item.categoryId" v-bind:index="item.categoryId.toString()">
              <template slot="title">
                <span>{{item.categoryContent}}</span>
              </template>

              <el-menu-item v-for="(item, index) in blogTitles" :key="item.blogId" v-bind:index="item.blogId.toString()">
